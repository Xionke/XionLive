(function() {
  ...
  function apiHlsProxy(url, referer) {
    return "/api/hls?url=" + encodeURIComponent(url) + "&referer=" + encodeURIComponent(referer || DEFAULT_REFERER);
  }
  ...
  async function resolveXionMatch(matchUrl, requestedStreamId) {
    ...
    if (isM3u8) {
      try {
        ...
        var playableUrl = apiHlsProxy(clean.href, referer);
    ...
    if (pageHtml.length < 100) throw new Error("Match page empty (" + pageHtml.length + " bytes)");
    try {
      var dataApiBaseUrl = parseDataApiBaseUrlFromPage(pageHtml);
    } catch(e) { throw new Error("No data API in page: " + e.message); }
    try {
      var streamSiteDigit = parseStreamSiteDigitFromPage(pageHtml, input);
    } catch(e) { throw new Error("No stream digit: " + e.message); }
    ...
    if (!sigResp.ok) throw new Error("Signature HTTP " + sigResp.status);
    ...
    if (!detailResp.ok) throw new Error("Match detail HTTP " + detailResp.status);
    ...
    if (!sdResp.ok) throw new Error("Stream detail HTTP " + sdResp.status);
    var sdHeaders = {};
    ...
    var playableUrl = apiHlsProxy(signedUrl, playerReferer);
